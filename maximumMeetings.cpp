...
bool comapre(struct meet a, struct meet b) {
...
vector<int> maximumMeetings(vector<int> &start, vector<int> &end) {
	...
	// 	create meeting array of size n
 	  for(int i = 0; i < n; i++) {
 	  	meeting[i].meetID = i + 1;
 	  ...
 	  sort(meeting, meeting + n, comapre);

 	  vector<int> result; 

 	  result.push_back(meeting[0].meetID);
 	  ...
 	  for(int i = 1; i < n; i++) {
 	  	...
 	  	if(meeting[i].startTime > currentTime) {
 	  		result.push_back(meeting[i].meetID);
